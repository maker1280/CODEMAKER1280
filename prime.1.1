...
//////////////////////////////////////
    {
      ...
      func: async (sprite, script) => { // 실행할 JS 코드
        // 여기가 바로 해결책!
        // 사용자가 %1에 입력한 실제 값을 'SEARCHRESULT'라는 이름으로 가져오는 거야!
        const actualInputValue = script.getValue('SEARCHRESULT', script); 
        
        // 그리고 eval() 함수에 이 실제 값을 넘겨주면 돼!
       try { eval(actualInputValue); 
       } catch (e)
       {
       alert ('유효하지 않습니다')
       }
...
//////////////////////////////////////
  ...
  alert('제작자 도금'),
 alert('prime 1.1')
LibraryCreator.start(blocks, 'API', '프라임 1.1')
